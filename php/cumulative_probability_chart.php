...
foreach ($parameters as $parameter) {
    ...
    JOIN ProbingSequenceOrder p on p.probing_sequence = w.probing_sequence
    $join_table_clause
    $where_clause " . ($sort_clause ? $sort_clause .", [X]" : "ORDER BY [X]");
        ...
        <div id="dropdownRangeMargin" class="z-10 hidden bg-white rounded-lg shadow w-60">
            <ul class="h-auto px-3 pb-3 overflow-y-auto text-sm text-gray-700" aria-labelledby="dropdownSearchButtonProbe">
                ...
                <li>
                    ...
                    <input type="range" id="marginRange" min="0" max="100" value="0" step="1" class="w-48 h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer">
                    <span id="rangeValue" class="text-sm font-semibold mt-2">0%</span>
                    </div>
                    </div>
                </li>
            </ul>
...
<h1 class="text-center text-2xl font-bold w-full mb-6">Cumulative Probability</h1>
...
foreach ($groupedData as $parameter => $data) {
    ...
    echo 'Probability of <b>' . $testNameX . '</b>';
    echo '</div>';
